...
function vmStatus() {
		aListStatus=$1		
			...
			if [ "$aListStatus" != "" ]; then
				...
				if [ "$name" != '"<inaccessible>"' ]; then
					echo $tmp | awk -F'|' '{ printf "\033[1;30m%-20s \033[1;35m%-17s \033[1;30m%5d \033[1;34m%-15s \033[1;33m%-50s \033[1;31m%s \n", $1,$2, $3, $4, $5, $6}'
				else
					echo $tmp | awk -F'|' '{ printf "\033[1;30m%-20s \033[1;31m%-s\n", $1,$6}'
				fi
